Pradaria.Game = function (game) {
    // Propriedades do Phaser
    this.game;      //  Uma referência ao jogo em execução
    this.add;       //  Utilizado para adicionar sprites, textos etc
    this.camera;    //  Câmera do jogo
    this.cache;     //  Cache do jogo
    this.input;     //  Utilizado para ter acesso à tecla do jogador
    this.load;      //  Pré-carregamento dos assets
    this.math;      //  Operadores matemáticos
    this.sound;     //  Gerenciador de música - adicionar, tocar etc
    this.stage;     //  Nível do jogo
    this.time;      //  Tempo
    this.tweens;    //  Tween Manager
    this.state;     //  State Manager
    this.world;     //  Inicia o mundo
    this.particles; //  Gerenciador de partículas
    this.physics;   //  Gerenciador da física do jogo
    this.rnd;       //  Gerador de números aleatórios
};

Pradaria.Game.prototype = {
    create: function () {

        // Define o tamanho do mundo e adiciona o fundo
        var worldSize = 1920;
        ...
        // Criação do jogador e sua função de câmera
        ...
        // Define a câmera que acompanha o jogador
        this.showLabels();
    },

    // Verifica ações e alterações
    update: function () {
        ...
        this.goldLabel.text = this.gold + ' Ouro';
    ...
    playerHandler: function () {
        ...
        if (this.player.alive) {
            ...
            // Ataca quando o botão do mouse é pressionado
            ...
            // Usa o especial quando o espaço é pressionado
            if (this.game.time.now > this.spellCooldown) {
                this.spellLabel.text = "ESPECIAL CARREGADO!";
                ...
                this.spellLabel.text = "RECARREGANDO...";
            }
    ...
    enemyHandler: function () {
        ...
        this.enemies.forEachDead(function (enemy) {
            if (this.rng(0, 5)) {
                ...
                this.notification = 'O ' + enemy.name + ' deixou cair uma poção!';
            }
    ...
    bossHandler: function () {
        ...
        // Faz o spawn do boss se o jogador tiver moedas o suficiente
        if (this.gold > this.goldForBoss && !this.bossSpawned) {
            ...
            this.notification = 'Um' + boss.name + ' apareceu!';
        }
        ...
        this.bosses.forEachDead(function (boss) {
            ...
            this.notification = 'O ' + boss.name + ' deixou cair uma poção!';
            this.xp += boss.reward;

            // Faz o dragão explodir - Sprite de vitória contra o dragão
    ...
    showLabels: function () {
        ...
        this.healthLabel = this.game.add.text(window.innerWidth / 2 - 65, this.game.height - 50, text, style);
        ...
        this.goldLabel = this.game.add.text(this.game.width - 105, this.game.height - 25, text, style);
        ...
        this.spellLabel = this.game.add.text(window.innerWidth / 2 - 85, this.game.height - 17, text, style);
    ...
    levelUp: function () {
        ...
        this.notification = this.player.name + ' avançou para o level ' + this.player.level + '!';
    ...
    attack: function (attacker, attacks) {
        ...
        if (attacker.alive && this.game.time.now > attacks.next && attacks.countDead() > 0) {
            ...
            console.log(attacker.name + " usou " + attacks.name + "!");
    ...
    generateAttacks: function (name, amount, rate, range) {
        ...
        // Criação da animação dos ataques
    ...
    hit: function (target, attacker) {
        ...
        if (this.game.time.now > target.invincibilityTime) {
            ...
            this.notification = attacker.name + ' causou ' + attacker.strength + ' dano ao ' + target.name + '!';
    ...
    collect: function (player, collectable) {
        ...
        if (!collectable.collected) {
            ...
            if (collectable.name === 'gold') {
                ...
                this.goldSound.play();
                this.notification = 'Você pegou ' + collectable.value + ' ouro(s).';
                collectable.destroy();
            } else if (collectable.name === 'chest') {
                collectable.animations.play('open');
                this.gold += collectable.value;
                this.goldSound.play();
                this.notification = 'Você abriu um báu e encontrou ' + collectable.value + ' ouros!';
                collectable.lifespan = 1000;
            } else if (collectable.name === 'healthPotion') {
                player.health += collectable.value;
                this.notification = 'Você usou uma poção, te curando ' + collectable.value + ' de vida!';
                this.potionSound.play();
                collectable.destroy();
            } else if (collectable.name === 'vitalityPotion') {
                player.vitality += collectable.value;
                this.notification = 'Você usou uma poção, aumentando sua vitalidade em ' + collectable.value + '!';
                this.potionSound.play();
                collectable.destroy();
            } else if (collectable.name === 'strengthPotion') {
                player.strength += collectable.value;
                this.notification = 'Você usou uma poção, aumentando sua força em ' + collectable.value + '!';
                this.potionSound.play();
                collectable.destroy();
            } else if (collectable.name === 'speedPotion') {
                player.speed += collectable.value;
                this.notification = 'Você usou uma poção, aumentando sua velocidade em ' + collectable.value + '!';
                this.potionSound.play();
                collectable.destroy();
            }
        }
    ...
    generateEnemy: function () {
        ...
        console.log('Gerou ' + enemy.name + ' com ' + enemy.health + ' de vida, ' + enemy.strength + ' de força, e ' + enemy.speed + ' de velocidade.');
    ...
    generateSkeleton: function (enemy) {
        ...
        return this.setStats(enemy, 'Esqueleto', 100, 70, 20, 5, 6);
    },
    ...
    generateBat: function (enemy) {
        ...
        return this.setStats(enemy, 'Morcego', 20, 200, 10, 2, 8);
    },
    ...
    generateGhost: function (enemy) {
        ...
        return this.setStats(enemy, 'Fantasma', 200, 60, 30, 7, 9);
    },
    ...
    generateSpider: function (enemy) {
        ...
        return this.setStats(enemy, 'Aranha', 50, 120, 12, 4, 10);
    },
    ...
    generateDragon: function (colorIndex) {
        ...
        console.log('Spawnou um dragão!');

        return this.setStats(boss, 'Dragão', 2000, 100, 50, 500, 0);
    },
    ...
    playSound: function (name) {
        ...
        } else if (name === 'Morcego') {
            this.batSound.play();

        } else if (name === 'Fantasma') {
    ...
    gameOver: function () {
        ...
        // Após derrota do jogador, destrói todos os objetos
        ...
        // Volta para o menu principal após derrota
